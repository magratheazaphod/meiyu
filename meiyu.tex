...
For each day from 1 January 1951 to 31 December 2007 (20,819 total), our Meiyu algorithm determines. In addition, some days feature two frontal events, each of which are separately detected. The algorithm follows the subsequent checklist:

\begin{enumerate}
	\item At each longitude, we find the latitude .We find the lat If a 5\textdegree swath (20 points) of maxima exceeds 10 mm/day, we move to step 2. Otherwise it is a no-Meiyu day.
	
	\item Given a preliminary fit, we perform a weighted least-squares linear fit of the \textit{latitudes} with the intensity as weight.
	
	\item A recursive algorithm. Using the initial fit as guideline, we select a new set of maxima within \textit{k} degrees of the best fit line. $k$ is progressively decreased with each iteration.
	
	\item We now have a best fit estimate. This allows us to calculate the ``quality score'' $Q_1$, which equals the fraction of total daily precipitation inside our domain that falls within 5 degrees of our best fit.
	
	\item The algorithm now looks for a secondary front as follows:
	
	\begin{enumerate}
		
		\item All precipitation within the primary front is removed. The front criterion from Step 1 is repeated. If passed, steps 2-4 are repeated.
		
		\item If a secondary Meiyu is found, we also calculate $Q_2$, which is the fraction of precip inside of the secondary front \textit{with primary front removed}, and $Q_{1\_alt}$, which is precip frac inside first front \textit{with secondary front removed}.
		
	\end{enumerate} 
	
	\item The day is now classified as either a primary Meiyu day, a primary AND secondary Meiyu day, or a no-Meiyu day. We use the following criteria:
	
	\begin{enumerate}
		\item We define the ``Taiwan fraction'' (TW) as the percentage of daily rainfall that falls over Taiwan. If TW > 20\%, there is no Meiyu. We do this because...
		
		\item If $Q_1 > .6$, the primary Meiyu is counted. If there is a secondary front and $Q_2 >.6$, the secondary front is also counted.
		
		\item If $Q_1 < .6$, then we check if both $Q_{1\_alt}$ and $Q_2$ $>$ .6. In these (rare) cases, both fronts are counted in our statistics. Otherwise, it is a no-Meiyu day. This allows us to find double front days where the original $Q_1$ was too low.
...
\subsection{Defining Meiyu season}	
	...
	It can be seen that mean rainfall and Meiyu occurrence are not entirely equivalent, since the Southern China peak in August does not correspond to a surge in Meiyu events.
