...
\subsection{Statistical Significance of Changes in Means}
	...
	Two separate Monte Carlo methods are employed to assess the statistical significance of changes in means between time periods: A permutation test, and bootstrapping for the difference in means between each time period. Given $n_1$ and $n_2$ observations for time periods 1 and 2 respectively, the permutation test creates new samples of sizes $n_1$ and $n_2$ \textit{without replacement} such that all observations are used. We then take the difference in the mean of each new sample as . This allows for the direct calculation of a $p$ value:
	.  In the former method, the full set of observations is partitioned Past authors have suggested that a permutation test is a stricter test of significance than the bootstrapping methods (INSERT CITATION). However, in our analysis, it is found that the two almost always produce very similar results. This tests of statistical significance. In the subsequent presentation of results, we have presented the result of the permutation test.
...
%should we add the mean annual cycle somewhere too?
% might be worth plotting the whole year for the time subsets too. 
...
\section{Comparison to previous work: the South Flood-North Drought}
...
Many potential mechanisms have been suggested for the South Flood North Drought. \citet{Qu2012} proposed that changes in Indian Ocean SST lead to meridional shifts in the jet over East Asia.
